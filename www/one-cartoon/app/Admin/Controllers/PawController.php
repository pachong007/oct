...
class PawController extends Controller
{
    ...
    private $SourceComicRenewTASK = "source:comic:renew:task";
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.code === 0){
                    ...
                    x(that.Dom2.firstChild,"漫画增量任务","{$this->SourceComicRenewTASK}")
...
<table id='kk_task' class='table table-hover grid-table'><tbody></tbody></table>"))
    ...
    public function getPawCache(Request $request)
    {
        ...
        if($request->has('stop')){
            ...
            $data = [
                ...
                $this->SourceComicRenewTASK => $this->getCacheList($redis, $this->SourceComicRenewTASK),
    ...
    private function getCacheList($redis, $key)
    {
        ...
        switch ($key) {
            ...
            case $this->SourceComicRenewTASK:
            ...
            case $this->SourceImageTASK:
                ...
                $data['list'] = $redis->lrange($key, 0, 7);
                break;
            case $this->TaskStepRecord:
                $data['len'] = $redis->llen($key);
                $data['list'] = $redis->lrange($key, $data['len'] - 7, -1);
                break;
    ...
    public function setPawCache(Request $request)
    {
        ...
        switch ($cache){
            ...
            case $this->SourceComicRetryTask:
                ...
                $list = $redis->lrange($this->SourceComicTASK,0,-1);
                $newList = array_unique($list);
                $redis->del($this->SourceComicTASK);
                foreach ($newList as $id){
                    $redis->rpush($this->SourceComicTASK,$id);
                }
                break;
            case $this->SourceChapterRetryTask:
                while (true){
                ...
                $list = $redis->lrange($this->SourceChapterTASK,0,-1);
                $newList = array_unique($list);
                $redis->del($this->SourceChapterTASK);
                foreach ($newList as $id){
                    ...
                    if($chapter) {
                        ...
                        $redis->rpush($this->SourceChapterTASK, $id);
                    }
                }
                break;
        }
